...
void SidFile::Load(QString filename, QString path)
{
    ...
    //If load address is empty, use first two bytes from song data instead
    if(m_blob.at(8) == 0 && m_blob.at(8+1) == 0) {
        unsigned short dataOffset = m_blob.at(6)<<8 | m_blob.at(6+1)<<0;
        //Song data use little endian
        m_loadAddress = (unsigned char)m_blob.at(dataOffset+1)<<8 | (unsigned char)m_blob.at(dataOffset)<<0;
    } else {
